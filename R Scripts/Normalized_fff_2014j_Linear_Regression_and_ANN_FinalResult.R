...
df5_final_result <- subset(df4, select = -c(id_student, disability, age_band, imd_band, highest_education, region, gender, TMA, CMA))

# Convert categorical values to numeric values

df5_final_result$final_result <- as.numeric(factor(df5_final_result$final_result , levels=c("Distinction","Fail","Pass","Withdrawn")))

# Normalize Final Result Label
final_result_norm <- data.frame(scale(df5_final_result$final_result, center = TRUE, scale = TRUE))

# Append normalized label column to data set

df5_final_result <- data.frame(df5_final_result, final_result_norm)

# Remove Final Result Column
df5_final_result <- subset(df5_final_result, select = -c(X, final_result))

# Rename Columns

names(df5_final_result) <- c("dataplus","dualpane","forumng","glossary","homepage","htmlactivity","oucollaborate","oucontent","ouwiki","page","questionnaire","quiz","repeatactivity","resource","subpage","url","num_of_prev_attempts","studied_credits","final_result")
...
pr.nn <- compute(nn,final_result_Test[,1:18])
