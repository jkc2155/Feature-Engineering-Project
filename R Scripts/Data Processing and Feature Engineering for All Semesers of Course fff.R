...
## Create data frames for course "FFF." ## 

assessment_fff <- subset(assessments_df, code_module %in% "FFF", select = c(id_assessment, assessment_type, date, weight))

std_info_df_fff <- subset(std_info_df, code_module %in% "FFF", select = c(id_student, gender, region, highest_education, imd_band, age_band, num_of_prev_attempts, studied_credits, disability, final_result))

std_registration_fff<- subset(std_registration_df, code_module %in% "FFF", select = c(id_student, date_registration, date_unregistration))

std_vle_fff <- subset(std_vle_df, code_module %in% "FFF", select = c(id_student, id_site, date, sum_click))

vle_fff <- subset(vle_df, code_module %in% "FFF", select = c(id_site, activity_type))
...
combined_assessments_fff <- merge(std_assessments_df, assessment_fff, by = "id_assessment", all = FALSE)

# Create new data frame by combining Assessment data with Student VLE data
std_assessment_vle_fff <- merge(combined_assessments_fff, std_vle_fff, by= "id_student")

# Create new data frame by student vle information data with vle discription data
std_assessment_vle_fff <- merge(std_assessment_vle_fff, vle_fff, by= "id_site")

# Create data frame of average score for specific assessment type of each student. 
assessment_score_avg_fff <- std_assessment_vle_fff %>% dplyr::group_by(assessment_type, id_student) %>% dplyr::summarise(mean(score))

# Create table with average number of clicks per student and activity type
activity_type_sum_click_fff <- std_assessment_vle_fff %>% dplyr::group_by(activity_type, id_student) %>% dplyr::summarise(mean(sum_click))

# Spread new tables
spread_assessment_core_avg_fff <- tidyr::spread(assessment_score_avg_fff, assessment_type, 'mean(score)')

spread_activity_type_avg_click_fff <- tidyr::spread(activity_type_sum_click_fff, activity_type, 'mean(sum_click)')

# Merge student data and average data and feature together
df <- merge(spread_activity_type_avg_click_fff, spread_assessment_core_avg_fff, by = "id_student")
df <- merge(df, std_info_df_fff, by ="id_student")
df <- merge(df, std_registration_fff, by = "id_student")
...
names(df3) <- c("id_student","dataplus","dualpane","folder","forumng","glossary","homepage","htmlactivity","oucollaborate","oucontent","ouelluminate","ouwiki","page","questionnaire","quiz","repeatactivity","resource","subpage","url","CMA","TMA","num_of_prev_attempts","studied_credits","final_result","disability","age_band","imd_band","highest_education","region","gender")
...
write.csv(df3, file = "Normalized_fff_Dataset.csv")
...
write.csv(df3_na_none, file = "Normalized_fff_Dataset_NA_Removed.csv")
